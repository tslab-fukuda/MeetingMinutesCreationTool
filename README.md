...
録音で取得した文字起こしは、LLM で要約して現在の挿入先 `\item` に反映できます。

使い方:

- TeX エディタで挿入したい `\item` 付近にカーソルを置き、`カーソル位置を挿入先` を押します。
- `自動反映` が ON の場合、停止ボタンを押した後に録音全体を文字起こしし、内容に応じた数の `\item` に分割要約して現在の挿入先へ反映します。
- 挿入先を手動で変えたい場合は、`次項目へ` を押すと次の空の `\item` に移ります。
- `自動反映` を OFF にしている場合や手動で反映したい場合は、`要約を反映` を押します。

補足:

- 録音は開始から停止までを1つの音声として扱い、15秒ごとの要約挿入は行いません。
- 挿入先は空の `\item` を優先して選びます。
- 要約には `LLM_PROVIDER` で選択された LLM を使います。失敗した場合は短く整形した文字起こしを挿入します。
