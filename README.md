...
`話題ごと要約` が ON の場合は、15 秒ごとの文字起こしセグメントをいったん蓄積し、話題が切り替わったと判定した時点で、それまでの会話をまとめて要約・出力します。これにより、15 秒の区切りで途中まで話された会話も、ひとまとまりの話題として要約されます。

補足:

- 手入力した会話文を要約する入力欄は使いません。
- 要約結果は内容に応じて複数の `\item` に分かれます。
- 話題ごと要約では、新しい 15 秒セグメントが届くたびに「直前まで蓄積した会話と同じ話題か」を LLM で判定します。同じ話題なら出力せずに蓄積を続け、話題が切り替わったと判定した時点で、それまで蓄積した会話を 1 つの話題として要約し、項目を追加します。
- 話題が長く続いて切り替わらない場合でも、一定数(約 24 セグメント≒6 分)蓄積したら安全のため自動で区切って要約します。
- 録音を停止すると、まだ要約していない最後の話題があれば自動で要約します。
- 右側の `選択セクションへの挿入先` には、まだ要約していない蓄積中のセグメント数も表示されます。
...
- API（ChatGPT / ローカルLLM の音声API）での文字起こし時には `会議の文字起こし。固有名詞を丁寧に扱う。` という補助プロンプトを渡します。ローカルの faster-whisper では、このプロンプトが冒頭セグメントの出力に混入する不具合があったため、補助プロンプトは渡しません。
- `文字起こし整形` が ON の場合、各セグメントの文字起こし結果を LLM で読みやすく整えてから、ライブ文字起こしと TeX への反映に使います（意味は変えず、誤認識・句読点・スペースを整えます）。OFF にすると素のままの文字起こしを表示します。LLM が未設定のときは整形をスキップして素の結果を表示します。
...
- 録音の文字起こしで `ローカルLLM` を選ぶ場合、音声からの文字起こしはローカルの faster-whisper で行い、その結果テキストの要約だけをローカル推論 API に送ります。そのため、ローカル推論 API は Chat Completions 互換であれば十分で、音声文字起こし(Audio Transcriptions)対応は不要です。
- このとき `LOCAL_TRANSCRIBE_MODEL` は使われません(文字起こしは faster-whisper が担当します)。文字起こしモデルは下記の `LOCAL_WHISPER_MODEL` で決まります。

## ローカル文字起こしモデル(精度)

`Local` と `ローカルLLM` の文字起こしは、ローカルの faster-whisper で行います。使うモデルは `local_settings.env` で変更できます。
...
- `tiny` … 最速だが日本語精度は非常に低い(旧既定)。
- `small` … 精度と CPU 速度のバランスが良い(現在の既定、約 500MB)。日本語会議ならまずこれ。
- `medium` … `small` より高精度だが CPU では重い(約 1.5GB)。
- `large-v3` … 最高精度だが CPU では実用的でない(約 3GB)。

補足:

- 既定は CPU 実行(`device=cpu`, `compute_type=int8`)です。
- NVIDIA GPU がある場合は `LOCAL_WHISPER_DEVICE=cuda` と `LOCAL_WHISPER_COMPUTE_TYPE=float16` にすると、`medium` / `large-v3` も高速に使えます(別途 CUDA / cuDNN の導入が必要な場合があります)。
- モデルは初回録音時に自動ダウンロードされます(`small` で約 500MB)。初回はダウンロードとモデル読み込みのぶん時間がかかります。
- 精度が足りない場合は、まず `LOCAL_WHISPER_MODEL` を `medium` に上げるのが効果的です。`文字起こし整形` の LLM は文章を整えるだけで、聞き間違えた単語までは復元できません。
